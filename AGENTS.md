...
CoderProxy = 牛码中转：借鉴参考实现逆向出的「伪装牛码登录 + 请求指纹」，以 **OpenAI 兼容 /v1** 暴露给任意 agent 的本地中转服务。

**开发文档全在** **`docs/`。开始时先读** **`docs/README.md`（索引 + 自举环境）。**

## 硬性规则（必守，违反即视为返工）

1. `relay/app/**` 源自参考实现的本地拷贝，**已脱离原项目独立演进**：可直接修改本仓库代码，修改处加「本地修订」注释说明与原实现的差异（示例见 `relay/app/models/providers/ta3.py`）。
