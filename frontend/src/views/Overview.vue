...
async function loadConfig() {
  try {
    ...
    console.error("[overview] loadConfig", e);
  }
}
...
onMounted(() => {
  ...
  configTimer = window.setInterval(loadConfig, 5000);
  ...
  <div>
    ...
    <div class="grid2-fixed">
      ...
      <div class="card">
        ...
        <div class="kpi-grid">
          <div class="kpi">
            <div class="kpi-num" style="color: var(--cp-cyan)">{{ requests.toLocaleString() }}</div>
            <div class="kpi-label">请求数</div>
          </div>
          <div class="kpi">
            <div class="kpi-num" style="color: var(--cp-green)">{{ done.toLocaleString() }}</div>
            <div class="kpi-label">完成数</div>
          </div>
          <div class="kpi">
            <div class="kpi-num" style="color: var(--cp-red)">{{ error.toLocaleString() }}</div>
            <div class="kpi-label">错误数</div>
          </div>
          <div class="kpi">
            <div class="kpi-num" style="color: var(--cp-yellow)">{{ refresh.toLocaleString() }}</div>
        ...
        <table v-else class="tbl">
          ...
          <tbody>
            <tr v-for="e in recentEvents" :key="e.id">
              ...
              <td>
                <span v-if="kindMeta[e.kind]" class="cp-tag" :class="kindMeta[e.kind][1]">{{ kindMeta[e.kind][0] }}</span>
                <span v-else class="cp-tag gray">{{ e.kind }}</span>
              </td>
